            ...
            <div class="bg-gray-900 border border-purple-900/50 overflow-hidden shadow-2xl rounded-2xl">
                <div class="p-8">
                    <h2 class="text-2xl font-black text-white uppercase tracking-tighter mb-6">
                        Nuevo <span class="text-purple-500">Usuario</span>
                    </h2>
                    ...
                    <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="password" class="block text-xs font-bold text-purple-400 uppercase mb-2">Contraseña</label>
                                <input type="password" name="password" id="password" required
                                    class="w-full bg-gray-800 border-gray-700 text-white rounded-xl focus:ring-purple-500 focus:border-purple-500 transition shadow-inner">
                            </div>
                            <div>
                                <label for="password_confirmation" class="block text-xs font-bold text-purple-400 uppercase mb-2">Confirmar Contraseña</label>
                        ...
                        <div>
                            ...
                            <select name="role" id="role" 
                                class="w-full bg-gray-800 border-gray-700 text-white rounded-xl focus:ring-purple-500 focus:border-purple-500 transition shadow-inner">
                                <option value="clubber" {{ old('role') == 'clubber' ? 'selected' : '' }}>Clubber (Usuario Estándar)</option>
                                <option value="organizer" {{ old('role') == 'organizer' ? 'selected' : '' }}>Organizer (Promotor)</option>
                                <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>Admin (Control Total)</option>
                            </select>
                        </div>
